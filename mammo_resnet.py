...
# Runs a binary classification between cancer and noncancer patches without GAN data augmentation
# Resnet architecture
...
X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(mgram_data,mgram_labels,test_size=0.1,random_state=10)
...
#            batch_index = np.random.choice(draw,size=batch_size,replace=False)
        ...
        if i % 2000 == 0:
            ...
            #yT = np.reshape(np.array(xT['labels'])[point:point+500],[500])
            yT = yT[:] - 1
